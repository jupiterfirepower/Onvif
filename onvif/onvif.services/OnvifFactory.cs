...
namespace onvif.services {
	...
	public class OnvifFactory 
    {
        ...
        private double _closeTimeOut = 30.0;
        private double _openTimeOut = 30.0;
        private double _sendTimeOut = 10.0;
        private double _receiveTimeOut = 3.0;
	    private string BindingCloseTimeOutAppKey = "BindingCloseTimeOut";
        private string BindingOpenTimeOutAppKey = "BindingOpenTimeOut";
        private string BindingSendTimeOutAppKey = "BindingSendTimeOut";
        private string BindingReceiveTimeOutAppKey = "BindingReceiveTimeOut";

        private void ReadConfigParameters()
	    {
            var bindingCloseTime = ConfigurationManager.AppSettings[BindingCloseTimeOutAppKey];
	        if (!string.IsNullOrWhiteSpace(bindingCloseTime))
	            double.TryParse(bindingCloseTime, out _closeTimeOut);

            var bindingOpenTimeOut = ConfigurationManager.AppSettings[BindingOpenTimeOutAppKey];
            if (!string.IsNullOrWhiteSpace(bindingOpenTimeOut))
                double.TryParse(bindingOpenTimeOut, out _openTimeOut);

            var bindingSendTimeOut = ConfigurationManager.AppSettings[BindingSendTimeOutAppKey];
            if (!string.IsNullOrWhiteSpace(bindingSendTimeOut))
                double.TryParse(bindingSendTimeOut, out _sendTimeOut);

            var bindingReceiveTimeOut = ConfigurationManager.AppSettings[BindingReceiveTimeOutAppKey];
            if (!string.IsNullOrWhiteSpace(bindingReceiveTimeOut))
                double.TryParse(bindingReceiveTimeOut, out _receiveTimeOut);
	    }
		...
		public OnvifFactory(bool mtomEncoding, bool wsAddressing, bool securityToken, bool useTls)
		{
            ...
            binding.CloseTimeout = TimeSpan.FromSeconds(_closeTimeOut);
            binding.OpenTimeout = TimeSpan.FromSeconds(_openTimeOut);
