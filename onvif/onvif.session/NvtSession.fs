namespace odm.core
    ...
    type INvtSession = interface
        ...
        abstract eventChannelList : List<IClientChannel>
...
//            return! TryCreateSession(uris)
        ...
        member private this.ChannelList = new List<IClientChannel>()
        member private this.EventChannelList = new List<IClientChannel>()

        member this.DisposeChannels() = 
            this.ChannelList.ForEach(fun x -> x.Dispose())
            this.ChannelList.Clear()

        member this.DisposeEventChannels() = 
            this.EventChannelList.ForEach(fun x -> x.Dispose())
        ...
        member this.CreateSession(deviceUri:Uri) = 
            ...
            let GetDeviceUnsecureClient = 
                let comp = Async.Memoize(async{
                    ...
                    //ToDo
                    this.ChannelList.Add(proxy :?> IClientChannel)
            ...
            let FixUrlCorrection(url:Uri) = 
                url.Relocate(url.Host, deviceUri.Port)
            ...
            let GetMediaClient = 
                let comp = Async.Memoize(async{
                    ...
                    if IsNull(xaddr) then
                        ...
                        let url = url.Relocate(deviceUri.Host, deviceUri.Port);
            ...
            let GetEventClient = 
                let comp = Async.Memoize(async{
                    ...
                    if IsNull(xaddr) then
                        ...
                        let endpointAddr = new EndpointAddress(FixUrlCorrection(url))
                        let proxy = factory.CreateChannel(endpointAddr)
                        //ToDo
                        this.EventChannelList.Add(proxy :?> IClientChannel)
                        do! SetupUserNameToken(proxy :?> IClientChannel)
...

            {
                new INvtSession with
                    ...
                    member this.eventChannelList = new List<IClientChannel>()
                        ...
                        return! Async.CreateWithDisposable(fun sink->
                            Async.StartImmediate(async{
                                try
                                    ...
                                    if  url.Port <> deviceUri.Port then
                                        let url = url.Relocate(url.Host, deviceUri.Port)
                                        let refParams = 
                    ...
                    member this.CreateBaseSubscription(consumerReference:EndpointReferenceType1, filter:FilterType, initialTerminationTime:string, subscriptionPolicy:SubscribeSubscriptionPolicy) = async{
                        ...
                        //Console.WriteLine("submanUri - {0}", submanUri.AbsoluteUri);
                        let! fixUri = FixUrl(new Uri(subscription.SubscriptionReference.Address.Value, UriKind.RelativeOrAbsolute))
                        //Console.WriteLine("FixUrl fixUri - {0}", fixUri.AbsoluteUri);
                        let fixUrl = new Uri(subscription.SubscriptionReference.Address.Value, UriKind.RelativeOrAbsolute);
                        let fixUri = fixUri.Relocate(deviceUri.Host, deviceUri.Port)
                        //Console.WriteLine("FixUri fixUri - {0}", fixUri.AbsoluteUri);
                        let submanUri = fixUri
                        let refParams = 
