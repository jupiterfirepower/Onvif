...
namespace onvif.utils 
    ...
    open onvif.utils
    open onvif.utils.NetMaskHelper
    ...
    module Async =
        ...
        let DoWorkAfterDelayInSeconds (f: unit -> Async<unit>) (intervalSeconds: int)  =
          async {
                    do! Task.Delay(System.TimeSpan.FromSeconds((float intervalSeconds))) |> AwaitVoidTask
                    do! f()
                 }

        let DoWorkAfterDelay (f: unit -> Async<unit>) (interval: int)  =
          async {
                    do! Task.Delay(interval) |> AwaitVoidTask
                    do! f()
                 }
        ...
        let DoPeriodicWork (f: unit -> Async<unit>) (interval: int) (token: CancellationToken) =
         async {
             while not token.IsCancellationRequested do
                 do! f()
                 do! Task.Delay(interval, token) |> AwaitVoidTask
         }
        ...
        member this.FtpDownload(uri:Uri):Async<Stream> = async{
            ...
            try
                ...
                do! async{
                    ...
                    //let! _response = (!request).AsyncGetResponse()
                    //use response1 = _response:?>FtpWebResponse
                    response := _response
...
//        }
    ...
    //                do! Async.Sleep(upgradeInfo.UploadDelay)
                ...
                return raise (new Exception("firmware upgrade not supported"))
        }
        ...
        member private this.Disposables = new List<SerialDisposable>()
        member val Flag = true with get, set
        member val StartProcessing : Nullable<System.DateTime> = new Nullable<System.DateTime>() with get, set

        member val Subman : ISubscriptionManagerAsync = null with get, set
        member val CheckConnectionDisposable : IDisposable = null with get, set

        member public this.Unsubscribe() =
            let unsubscribe(subman:ISubscriptionManagerAsync)=async{
                if subman |> NotNull then
                            try
            ...
            Async.StartImmediate(unsubscribe(this.Subman))

        member public this.Dispose() =
               try
                  if this.CheckConnectionDisposable <> null then this.CheckConnectionDisposable.Dispose()
                  this.Unsubscribe()
               with err->
                    dbg.Error(err)
            
        static member GetAppSettings (key : string) =
                ConfigurationManager.AppSettings.Item(key)

        static member val ExecuteWithTimeLimit = 
               let value = OdmSession.GetAppSettings("OdmSessionExecuteWithTimeLimit")
               if  not (value |> String.IsNullOrWhiteSpace) then
               try
                   float value
               with err->
                    10.0
        ...
        static member val XsDurationTimeOut = 
               let value = OdmSession.GetAppSettings("XsDurationTimeOut")
               if  not (value |> String.IsNullOrWhiteSpace) then
               try
                   float value
               with err->
        ...
        static member val BaseSuscriptionRenewInterval = 
               let value = OdmSession.GetAppSettings("BaseSuscriptionRenewInterval")
               if  not (value |> String.IsNullOrWhiteSpace) then
               try
        ...
        static member val CreatePullPointSubscriptionTimeOutMiliseconds = 
               let value = OdmSession.GetAppSettings("TryCreatePullPointSubscriptionTimeOutInMiliseconds")
               if  not (value |> String.IsNullOrWhiteSpace) then
               try
                    System.Int32.Parse(value) 
               with err->
        ...
        static member val PullPointTerminationTime = 
               let value = OdmSession.GetAppSettings("PullPointTerminationTime")
               if  not (value |> String.IsNullOrWhiteSpace) then
                    value 
               else 
                    "PT600S" 
        ...
        static member val BaseSubscriptionTerminationTime = 
               let value = OdmSession.GetAppSettings("BaseSubscriptionTerminationTime")
               if  not (value |> String.IsNullOrWhiteSpace) then
                    value 
               else 
                    "PT60S" 
        ...
        static member val CheckConnectionAgentInterval = 
               let value = OdmSession.GetAppSettings("CheckConnectionAgentInterval")
               if  not (value |> String.IsNullOrWhiteSpace) then
               try
                    System.Int32.Parse(value) 
               with err->
                ...
                let comp = async{
                    try
                        ...
                        let rec main() = 
                            async{
                                do! async{
                                    this.Disposables.ForEach(fun x-> x.Dispose())
                                    ...
                                    let createPullPointSubscription = async { 
                                            return! session.CreatePullPointSubscription(filter, OdmSession.PullPointTerminationTime, null)
                                    }

                                    let tryCreatePullPointSubscription() = async {
                                        let! submanVal = withTimeout  createPullPointSubscription OdmSession.CreatePullPointSubscriptionTimeOutMiliseconds //10 seconds by default
                                        match submanVal with 
                                        | Some value -> this.Subman <- value
                                        | None -> this.Subman <- null // what to do here?
                                        return this.Subman = null
                                    }

                                    if this.Flag then
                                       this.Flag <- false
                                       let tryCount = ref 0
                                       while  !tryCount < 3 && tryCreatePullPointSubscription() |> Async.RunSynchronously  do tryCount.Value <- !tryCount + 1
                                   
                                       let! subman = if this.Subman = null then session.CreatePullPointSubscription(filter, OdmSession.PullPointTerminationTime, null) else async { return this.Subman } //PT86400S PT600S
                                       this.Subman <- subman
                                       this.StartProcessing <- System.Nullable DateTime.UtcNow
                                       disp.Disposable <- Disposable.Create(fun()->
                                                Async.StartImmediate(unsubscribe(subman))
                                       )
                                       this.Disposables.Add(disp)
                                       do! loop(subman, XsDuration.FromSeconds((float OdmSession.XsDurationTimeOut)), 1024) 
                                    else
                                        return ()
                                }
                                //let tmDuration = System.Int32.Parse(OdmSession.PullPointTerminationTime.Replace("PT","").Replace("S",""))
                                //Async.DoWorkAfterDelay (fun () -> async { main() |> ignore }) ((tmDuration - 30) * 1000) |> ignore
                        ...
                        and loop(subman, timeout, messageLimit) = 
                            async{
                                let! cont = async{
                                    try
                                            let canExecute = helper.CanExecuteWithTimeLimit(TimeSpan.FromSeconds((float OdmSession.ExecuteWithTimeLimit)))
                                            if canExecute then
            ...
            Observable.Create(fun (observer:IObserver<OnvifEvent>) ->
                ...
                let comp = async{
                    try
                        ...
                        let checkConnectionAgent = HandlingMailbox<_>.Start(fun inbox -> 
                        ...
                        let rec messageLoop = async{
                            ...
                            // process a message
                            try
                                let addr = Dns.GetHostEntry(Dns.GetHostName()).AddressList.First(fun f -> f.AddressFamily = System.Net.Sockets.AddressFamily.InterNetwork)
                                let! adr = CheckConnectivity(addr)
                                let exp = new System.Exception(String.Format("BaseEventsSubscription Connection to the camera {0} lost.", devUri))
                                match adr with 
                                      | Some x -> () //System.Console.WriteLine("normal connection")
                                      | None -> 
                                        let count = ref 0
                                        let errorCount = ref 0
                                        while !count < 3 do
                                            match adr with 
                                              | Some x -> count.Value <- !count + 1
                                              | None -> 
                                                count.Value <- !count + 1
                        ...
                        checkConnectionAgent.OnError.Add(observer.OnError)

                        let obs = Observable.Interval(TimeSpan.FromMilliseconds((float OdmSession.CheckConnectionAgentInterval)));
                        this.CheckConnectionDisposable <- obs.Subscribe(fun x -> if not cts.Token.IsCancellationRequested then checkConnectionAgent.Post("ping"))
                        ...
                        let address = 
                            //if AmazoneUtilsHelper.IsRunOnAmazoneServer() && not (IpAddressHelper.IsLocalAddress(devUri.Host)) 
                            if not (IpAddressHelper.IsLocalAddress(devUri.Host)) 
                            then IpAddressHelper.GetExternalIpAddress() 
                            else localIp.ToString()
                        
                        let consumerReference = sprintf "http://%s:%d/subscription-%d" (address.ToString()) (port) (GetSubscriptionId())
                        //Console.WriteLine("consumerReference - {0}", consumerReference);
